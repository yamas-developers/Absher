...
class _RestaurantScreenState extends State<RestaurantScreen>
  ...
  void didChangeDependencies() {
    ...
    if (storeType == BUSINESS_TYPE_RESTAURANT) {
                        ...
                        if (storeType == BUSINESS_TYPE_PHARMACY)
                          Expanded(
                              child: Consumer<TakeawayPharmaProvider>(
                                  builder: (context, pro, _) {
                            return RestaurantArea(
                                storeType: storeType, resProvider: pro);
                          }))
                        else
...
class RestaurantArea extends StatefulWidget {
  const RestaurantArea({
    ...
    required this.resProvider, this.catId = "",
  }) : super(key: key);
  ...
  log("NJ: refresh ${controller.isRefresh}");
    return SizedBox(
      ...
      child: SmartRefresher(
        ...
        onLoading: () async {
          print("MJ: loading in onLoading ${controller.isRefresh}");

          // _refreshController.load
          // bool res = await loadMoreData();
          if(!controller.isRefresh)
          widget.resProvider.getData(/*empty_list: false*/);
        },
        onRefresh: () async {
          print("MJ: loading in refresh ${controller.isRefresh}");
          widget.resProvider.reset();
        },
