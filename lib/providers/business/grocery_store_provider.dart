...
class GroceryStoreProvider extends PaginationProvider<Business>{
  ...
  getData({bool callingForMore = true, QueryParams? params = null}) async {
    logInfo("calling get data in store provider: ");

    bool flag = c_getData(callingForMore);
    if(flag){
      dynamic response = await MjApiService()
          .getRequest(MJ_Apis.get_all_restaurants+
          "?business_type=${BUSINESS_TYPE_STORE}&offset=${c_offset}&limit=${c_limit}&searched_text=${params?.searchedText ?? ""}");
